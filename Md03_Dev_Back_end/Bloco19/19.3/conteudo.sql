...
--  dos 100 filmes com o maior custo de substituição, do valor mais alto ao mais baixo, entre os filmes feitos para menores de idade
...
WHERE rating REGEXP '[G]' AND replacement_cost>=18 ORDER BY replacement_cost DESC, title LIMIT 100;

SELECT title, description, release_year, replacement_cost, rating FROM film
WHERE rating LIKE '%G%' AND replacement_cost>=18 ORDER BY replacement_cost DESC, title LIMIT 100;
